...
window.initSliders = function() {
	if (document.querySelector(".livedemo__slider")) new Swiper(".livedemo__slider", {
...
window.initMarqueeSwiper = function() {
	const marqueeSliders = document.querySelectorAll(".marquee-slider");
	if (!marqueeSliders.length) return;
	marqueeSliders.forEach((el) => {
		const wrapper = el.querySelector(".swiper-wrapper");
		const originalSlides = [...wrapper.children];
		const gap = el.dataset.slidesGap || 24;
		if (!wrapper || !originalSlides.length) return;
		const getSlidesPerView = () => {
			return window.innerWidth < 991.98 ? 2 : 5;
		};
		const cloneSlides = () => {
			wrapper.querySelectorAll(".is-clone").forEach((slide) => {
				slide.remove();
			});
			const minSlidesCount = getSlidesPerView() * 6;
			const currentOriginals = [...wrapper.children].filter((slide) => !slide.classList.contains("is-clone"));
			if (currentOriginals.length >= minSlidesCount) return;
			let index = 0;
			while (wrapper.children.length < minSlidesCount) {
				const clone = currentOriginals[index % currentOriginals.length].cloneNode(true);
...
document.querySelector("[data-fls-slider]") && window.addEventListener("load", () => window.initMarqueeSwiper());
//#endregion
